...
public class ElementsPublicNetworkManifestTests
{
	...
	[Fact]
	public void ReviewedCatalogIsExactlyTwoAndFailsClosedElsewhere()
	{
		...
		Assert.Equal(2, catalog.Select(manifest => manifest.ManifestId).Distinct(StringComparer.Ordinal).Count());
		Assert.Equal(2, catalog.Select(manifest => manifest.CanonicalCborSha256).Distinct(StringComparer.Ordinal).Count());
		Assert.Equal(2, catalog.Select(manifest => LowerHex(SHA256.HashData(manifest.ExportCanonicalCbor()))).Distinct(StringComparer.Ordinal).Count());
		Assert.Equal([2, 2], catalog.OrderBy(manifest => manifest.ManifestSchema).Select(manifest => manifest.ManifestSchema).ToArray());
