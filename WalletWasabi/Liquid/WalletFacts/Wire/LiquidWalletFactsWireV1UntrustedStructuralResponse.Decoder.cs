...
internal sealed partial class LiquidWalletFactsWireV1UntrustedStructuralResponse
{
	internal static bool TryDecodeUntrustedStructuralResponse(
		...
		Decoder.TryDecodeUntrustedStructuralResponse(frame, expectedSourceEpoch, out response, out errorCode);
	...
	private static class Decoder
	{
		...
		out LiquidWalletFactsWireErrorCode errorCode)
		{
			...
			try
			{
				...
				response = new LiquidWalletFactsWireV1UntrustedStructuralResponse(ownedFrame, transactionOffsets);
		...
		private ref struct WireReader
		{
			...
			public readonly bool IsAtEnd => Offset == _frame.Length;
			...
			{
				if (length < 0 || length > _frame.Length - Offset)
				{
					value = default;
					return false;
				}

				value = _frame.Slice(Offset, length);
				Offset += length;
				return true;
			}
		}
